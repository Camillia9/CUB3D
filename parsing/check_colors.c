...
/**
 * Assigne les valeurs RGB à la structure selon le type
 */
void assign_rgb_values(t_data *data, int r, int g, int b, int color_type)
{
    if (color_type == 1) // Floor
    {
        ...
        data->colors.floor_color = encode_rgb_color(r, g, b);
    }
    else if (color_type == 2) // Ceiling
    {
        ...
        data->colors.ceiling_color = encode_rgb_color(r, g, b);
    }
}
...
void parse_rgb_values(char *color_start, t_data *data, int color_type)
{
    ...
    assign_rgb_values(data, rgb_values[0], rgb_values[1], rgb_values[2], color_type);
}
